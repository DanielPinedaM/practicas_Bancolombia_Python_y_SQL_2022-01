...
SELECT /* t1.*, */ /* necesito todas las columnas de la tabla 1 proceso_generadores.informe_recu_2020102_v2 */
       ...
       TRIM(CAST(t1.modalidad AS STRING)) AS modalidad,
       TRIM(UPPER(CAST(t1.producto_ajustado AS STRING))) AS producto_ajustado,
       TRIM(UPPER(CAST(t1.sgto_ajustado AS STRING))) AS sgto_ajustado,
       TRIM(UPPER(CAST(t1.vic_ccial AS STRING))) AS vic_ccial,
...
/*
Ver los nombres y tipos de datos de las columnas 
de la tabla proceso_consumidores.LEFT_JOIN_punto_1
...
DESCRIBE proceso_consumidores.LEFT_JOIN_punto_1
...
SI NO ((clasificacion_de_cartera<>'COMER')
        AND (clasificacion_de_cartera IS NOT NULL) -- INCOMPLETO - NO ESTOY SEGURO DE ESTA CONDICION - CREO Q NO HAY Q ESCRIBIRLA, BORRELA (CREO)
        AND ((modalidad IS NULL)
              OR (modalidad='')
            )
        AND ((base='HIPOTECARIO')
...
/* 
Reemplazar casillas vacias y NULL segun ciertos condicionales
Crear la tabla final q contiene la solucion del punto 1


Las tablas temporales se anidan de la siguiente forma:

1) en el primer WITH ... AS (...) 
- se selecciona la tabla original
q contiene las casillas vacías ó NULL
- NO se usan CASE WHEN 

2) - Para cada columna se crea una tabla temporal usando WITH ... AS (...)
   - A partir del segundo (incluyendo el segundo) SI se usan CASE WHEN 

3) en el segundo WITH ... AS (...)
- selecciono FROM la tabla original q contiene las casillas vacías ó NULL

4) en el cuarto WITH ... AS (...)
- selecciono la tabla de 3)
  y asi sucesivamente con todos los CASE WHEN 

Ejemplo:

WITH tabla_temporal_1_nombre_tabla_con_casillas_vacias_o_NULL AS 
(
      SELECT ...
      FROM nombre_tabla_con_casillas_vacias_o_NULL
),

tabla_temporal_2_nombre_campo_llenar_NULL AS
(
      SELECT ...
           (CASE
                WHEN ...
                ELSE nombre_campo_llenar_NULL_1
           END) AS nombre_campo_llenar_NULL_1
      FROM tabla_temporal_1_nombre_tabla_con_casillas_vacias_o_NULL
),

tabla_temporal_3_nombre_campo_llenar_NULL AS
(
      SELECT ...
           (CASE
                WHEN ...
                ELSE nombre_campo_llenar_NULL_2
           END) AS nombre_campo_llenar_NULL_2
      FROM tabla_temporal_2_nombre_campo_llenar_NULL
),

tabla_temporal_4_nombre_campo_llenar_NULL AS
(
      SELECT ...
           (CASE
                WHEN ...
                ELSE nombre_campo_llenar_NULL_3
           END) AS nombre_campo_llenar_NULL_3
      FROM tabla_temporal_3_nombre_campo_llenar_NULL
)
SELECT ...
FROM tabla_temporal_4_nombre_campo_llenar_NULL
...
--
--WITH tabla_temporal_1_proceso_consumidores_LEFT_JOIN_punto_1 AS
--(
--      SELECT id_cliente,                           	
--             TRIM(UPPER(CAST(clasificacion_de_cartera AS STRING))) AS clasificacion_de_cartera,	  
...
--                        /* SIEMPRE se cumple esto, cuando:
--                        BASE='HIPOTECARIO'
--                        entonces
--                        clasificacion_de_cartera IS NOT NULL */
--                        AND (clasificacion_de_cartera IS NOT NULL)
...
--          END) AS modalidad
--
--      FROM tabla_temporal_1_proceso_consumidores_LEFT_JOIN_punto_1
...
--),
--
--tabla_temporal_6_banca_ajustada AS
--(
--      SELECT id_cliente,                           	
--             TRIM(UPPER(CAST(clasificacion_de_cartera AS STRING))) AS clasificacion_de_cartera,	          	
--             TRIM(CAST(modalidad AS STRING)) AS modalidad,        	
...
--
--      FROM tabla_temporal_5_vic_ccial
--      ORDER BY id_cliente ASC
--)
--SELECT id_cliente,
--       TRIM(UPPER(CAST(clasificacion_de_cartera AS STRING))) AS clasificacion_de_cartera,
...
--       amcdty,
--       cncdbi 
--/* la tabla_temporal_6_banca_ajustada 
--es la tabla FINAL despues de llenar los campos NULL y vacios,
--es la solucion del punto 1 */
--FROM tabla_temporal_6_banca_ajustada
--ORDER BY id_cliente ASC
--;
...
(
      SELECT id_cliente,                           	
          ...
          /* producto_ajustado */
          (CASE 
               WHEN ( ( (TRIM(CAST(modalidad_nuevo AS STRING))='1')
                         AND (modalidad_nuevo IS NOT NULL)
                         AND (TRIM(CAST(modalidad_nuevo AS STRING))<>'')
                      )
                      AND ((producto_ajustado IS NULL)
                            OR (TRIM(CAST(producto_ajustado AS STRING))='NULL')
                            OR (TRIM(CAST(producto_ajustado AS STRING))='')
...
(
      SELECT id_cliente,                           	
             ...
             producto_ajustado_anterior,
             producto_ajustado_nuevo,
             sgto_ajustado_anterior,
             sgto_ajustado_nuevo,
             TRIM(UPPER(CAST(vic_ccial AS STRING))) AS vic_ccial_anterior,                    	
             TRIM(UPPER(CAST(banca_ajustada AS STRING))) AS banca_ajustada,                     	
             TRIM(UPPER(CAST(base AS STRING))) AS base,
...
SELECT id_cliente,                           	
       ...
       TRIM(UPPER(CAST(vic_ccial_nuevo AS STRING))) AS vic_ccial, 
       TRIM(UPPER(CAST(banca_ajustada_nuevo AS STRING))) AS banca_ajustada,                  	                   	
       TRIM(UPPER(CAST(base AS STRING))) AS base,
