...
tablas_borrado=[' proceso_generadores.master_customer',' proceso_generadores.master',' proceso_generadores.base_GIS']
...
#Y:/29.Tablero_IFRS9_COLGAP_CAH/Insumos/Calendario.xlsx
#W:\GCIA_ESTRA_RECU_CARTERA\29.Tablero_IFRS9_COLGAP_CAH\Insumos
def Base_Festivos():
    dfCalendario = pd.read_excel(r'C:\Users\danpined\OneDrive - Grupo Bancolombia\5_ReporteriaYprovisiones\2_PORTAFOLIO IFRS9\2_descomprimido\PROCESO COMPRIMIDO\Portafolio IFRS9\Insumos\Calendario.xlsx',sheet_name='calendario')
    dfFestivos = pd.read_excel(r'C:\Users\danpined\OneDrive - Grupo Bancolombia\5_ReporteriaYprovisiones\2_PORTAFOLIO IFRS9\2_descomprimido\PROCESO COMPRIMIDO\Portafolio IFRS9\Insumos\Calendario.xlsx',sheet_name='Festivos')
    ...
    sp=Sparky('danpined', 'IMPALA_PROD')
    sp.subir_df(dfConsolidacion,"proceso_consumidores.fecha_base_maestra_2")
    
    
    def Query_Base_fechas(): 
        print('ENTROOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO')       
        Query_1_Generar_Base_fechas=""" Create table proceso_generadores.base_dias stored as parquet as 
        ...
        from proceso_consumidores.fecha_base_maestra_2
        ...
        from proceso_consumidores.fecha_base_maestra_2 t1
...
def Query_Base_GIS():        
    ...
    CREATE TABLE proceso_generadores.base_GIS stored AS parquet AS

    WITH base AS
    ( SELECT *,
            row_number() over (partition BY tipo_doc, num_doc
                                ORDER BY ingestion_year DESC , ingestion_month DESC , ingestion_day DESC) AS rn
    --FROM resultados_cap_analit_y_gob_de_inf.geo__dir_clientes) --resultados_cap_analit_y_gob_de_inf.geo__dir_clientes
    FROM proceso_generadores.geo__dir_clientes)
    SELECT tipo_doc,
        num_doc,
        ciudad,
        departamento,
...
def Master_Customer(year,month):
    ...
    Create table proceso_generadores.master_customer stored as parquet as 
    select * from (
    ...
    where  year="""+year+""" and month="""+str(int(month)+1)+""" and ingestion_day=(select dia from proceso_generadores.base_dias where ano="""+year+""" and mes="""+str(int(month)+1)+""" and dia_habil_no_dom_fest=1 limit 1 ) ) Consulta1
    where rn=1 """ 
...
def Ifrs9_Colgap_Base(ingestion_year,ingestion_month,ingestion_day):
    Query_Variables_Base=""" 
     CREATE TABLE   proceso_generadores.portafolio_ifrs9_202112_2 stored as parquet as    --proceso_generadores.portafolio_ifrs9_202109
    With 
    ...
    SELECT 
        ...
        ,ingestion_month
        ,ingestion_day
    from resultados_riesgos.master_credit_risk  --proceso _riesgos.master_credit_risk
   ...
   cast(num_doc as bigint) as num_doc
    ...
    from resultados_riesgos.master_credit_risk   --proceso_generadores.master_credit_risk
    ...
    Info_Master_customer as  (
        ...
        ,t2.genero_cli
        from Capa_final t1 
        left join proceso_generadores.master_customer t2 on t1.num_doc=t2.num_doc),
    ...
    SELECT max(cod_subsegm) as cod_subsegm ,subsegm FROM resultados_vspc_clientes.master_customer_data
    where ingestion_year="""+ingestion_year+""" and ingestion_month="""+ingestion_month+""" and ingestion_day= 10
    group by subsegm
    ),
    ...
    --where year="""+ingestion_year+"""  and ingestion_month="""+str(int(ingestion_month)+1)+""" and ingestion_day=ingestion_day=(select dia from proceso_generadores.base_dias where ano="""+ingestion_year+""" and mes="""+str(int(ingestion_month)+1)+""" and dia_habil_no_dom_fest=1 limit 1 )
    --        ),
    ...
    left join  proceso_generadores.base_GIS t7 on cast(t1.num_doc AS bigint)=cast(t7.num_doc AS bigint) and cast(t1.tipo_identificacion AS bigint)=cast(t7.tipo_doc AS bigint) 
    )
...
#tablas_borrado_fechas=['proceso_generadores.base_dias']
...
Borrado_Tablas(tablas_borrado)
Query_Base_GIS()
Master_Customer('2022','01')
Ifrs9_Colgap_Base('2022','01','31') # AQUI SE PONE LA FECHA DEL MES PASADO DEL Q ESTAMOS ACTUALMENTE