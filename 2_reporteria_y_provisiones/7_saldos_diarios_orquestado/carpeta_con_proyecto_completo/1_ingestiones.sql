...
/* FECHAS SALDOS DIARIOS */
    ...
    with actual as (
        ...
        --                            'yyyy-MM-dd') as hoy
          cast('2022-05-13' as timestamp) as hoy
        ...
        --                            'yyyy-MM-dd') as fecha_corte
         , adddate(cast('2022-05-13' as timestamp),-1) as fecha_corte
    ...
    , case 
       -- when t2.holiday_colombia = 1 then adddate(now(),-2)
         when t2.holiday_colombia = 1 then adddate(cast('2022-05-13' as timestamp),-2)
...
'2022-05-13' AS hoy ,
concat(CAST(ingestion_year as STRING), "-", cast( ingestion_month AS string), "-", cast(ingestion_day AS string)) AS fecha_corte ,
cast(NULL AS string) AS fecha_corte_ant
FROM resultados_riesgos.ceniegarc_preliminar
WHERE ingestion_year = 2022
AND ingestion_month = 1
-- where year = if(month(cast('2022-05-13' as timestamp))=1, year(cast('2022-05-13' as timestamp))-1, year(cast('2022-05-13' as timestamp)))
-- and ingestion_month < if(month(cast('2022-05-13' as timestamp))=1, 12,month(cast('2022-05-13' as timestamp)))ORDER BY ingestion_year DESC, ingestion_month DESC
LIMIT 1 ;
...
/* FECHAS TRM */
    ...
    where ingestion_year = if(month(cast('2022-05-13' as timestamp))=1,year(cast('2022-05-13' as timestamp))-1,year(cast('2022-05-13' as timestamp))) and ingestion_month <=  if(month(cast('2022-05-13' as timestamp))=1,12,month(cast('2022-05-13' as timestamp))-1)
