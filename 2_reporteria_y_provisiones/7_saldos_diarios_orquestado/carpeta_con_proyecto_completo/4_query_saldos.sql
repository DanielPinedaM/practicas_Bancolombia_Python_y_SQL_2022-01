...
/* Marca Cancelaciones y/o Castigos */
    ...
    t1.fuente
        ...
        , t1.moneda
        , t1.clasificacion
        , if(t1.nutitula = 0 or t1.nutitula is null, t1.sld_cap_final, 0) as sld_cap_final -- Cambio Nutitula
        , if(t1.nutitula = 0 or t1.nutitula is null, t1.sld_cap_final_dolares, 0) as sld_cap_final_dolares -- Cambio Nutitula
        , t1.nueva_altura_mora
        , if(t1.nutitula = 0 or t1.nutitula is null, t1.cv, 0) as cv -- Cambio Nutitula
        , if(t1.nutitula = 0 or t1.nutitula is null, t1.cv_dolares, 0) as cv_dolares -- Cambio Nutitula
        , if(t1.nutitula = 0 or t1.nutitula is null, t1.c90, 0) as c90 -- Cambio Nutitula
        , t1.banca
        , t1.modalidad
        , t1.plan
        , t1.pd
    ...
    t1.fuente
        ...
        , t1.ingestion_day
    from {processZone2}.insumo_saldos t1
    where ingestion_year=year(cast('{FECHA_CIERRE}' as timestamp)) and ingestion_month=month(cast('{FECHA_CIERRE}' as timestamp)) and ingestion_day=day(cast('{FECHA_CIERRE}' as timestamp))
    and apli not in ('2') -- Cambio Spiwack
    ),
...
/* Variaciones */
    ...
    where fuente = 'SALDOS_DIARIOS' and
    t1.ingestion_year=year(cast('{FECHA_CORTE_ANT}' AS TIMESTAMP))
    ...
    UNION ALL SELECT llave1,
                    ...
                    ingestion_day
    FROM obl_cn
     WHERE ingestion_year=year(cast('{FECHA_CIERRE}' AS TIMESTAMP))
     AND ingestion_month=month(cast('{FECHA_CIERRE}' AS TIMESTAMP))
     AND ingestion_day=day(cast('{FECHA_CIERRE}' AS TIMESTAMP))
     AND apli NOT IN ('2')
    )
    ...
    SELECT llave1
          ...
          , coalesce(sum(CASE
                           WHEN ingestion_year=year(cast('{FECHA_CIERRE}' AS TIMESTAMP))
                                AND ingestion_month=month(cast('{FECHA_CIERRE}' AS TIMESTAMP)) 
                                AND ingestion_day=day(cast('{FECHA_CIERRE}' AS TIMESTAMP)) THEN sld_cap_final
                       END),0) AS sld_cap_cierre
          , coalesce(sum(CASE
                           WHEN ingestion_year=year(cast('{FECHA_CIERRE}' AS TIMESTAMP))
                                AND ingestion_month=month(cast('{FECHA_CIERRE}' AS TIMESTAMP)) 
                                AND ingestion_day=day(cast('{FECHA_CIERRE}' AS TIMESTAMP)) THEN sld_cap_final_dolares
          ...
          , coalesce(sum(CASE
                           WHEN ingestion_year=year(cast('{FECHA_CIERRE}' AS TIMESTAMP))
                                AND ingestion_month=month(cast('{FECHA_CIERRE}' AS TIMESTAMP)) 
                                AND ingestion_day=day(cast('{FECHA_CIERRE}' AS TIMESTAMP)) THEN cv
                       END),0) AS cv_cierre
   FROM base
   WHERE apli NOT IN ('2')
