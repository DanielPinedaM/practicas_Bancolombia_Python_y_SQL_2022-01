...
lista_orden_primeras_columnas_df_respuesta = ["numero_identificacion",               
                                              "numero_obligacion",                    
                                               nombre_nueva_columna_excel_respuesta_1, # = max_altmora_a_corte_de_febrero_20_del_cliente_interna
...
elementos_2 =  r'C:\Users\danpined\OneDrive - Grupo Bancolombia\5_ReporteriaYprovisiones\4_Proyecto_requerimientos_FNG\2_excel\1_insumo\2_Requerimiento Bancolombia 12052022 (1) (Mayo_17) (GianPaul).xlsx'
...
for elementos_2 in lista_de_rutas_absolutas: # esto va a leer todos los N Excel de los insumos
     ...
     and nombre_tabla_lz_2               =="ceniegarc_lz"
      ...
      (
        SELECT CAST({} AS BIGINT) AS numero_identificacion,
               ...
               TRIM(CAST({} AS STRING)) AS {},
               (({}*100) + {}) AS fecha_de_desembolso
        FROM {}.{}
        WHERE CAST({} AS BIGINT)
        ...
        (
         CASE
             ...
             WHEN (CAST({} AS STRING) = '')
                   ...
                   OR ({} IS NULL)
                  THEN 'SIN INFORMACION' /*max_altamora_a_corte_de_febrero_20_del_cliente_externa = 'NULL'*/
  ...
  """
  Reemplazar columnas vacias y NULL del DataFrame
  
  consulta_1
  si esta vacio o NULL reemplazo por SIN CARTERA ACTIVA

  consulta_2
  SE SUPONE q esta columna (consulta SQL) nunca da vacio ni NULL
  pero de todas formas si esta vacio o NULL reemplazo por ERROR NO SE ENCONTRO INFORMACION

  consulta_3
  si esta vacio o NULL reemplazo por SIN CARTERA ACTIVA

  consulta_4
  si esta vacio o NULL reemplazo por SIN INFORMACION
  """
  
  # Ver el tipo de dato del DataFrame
  #print(df_respuesta.dtypes)

  # Convertir todo el DataFrame a tipo string
  df_respuesta = df_respuesta.astype("string")
  # Verificar q se haya cambiado a tipo string el tipo de dato del DataFrame
  #print(df_respuesta.dtypes)

  # Eliminar los espacios en blanco al principio y al final de todo el DataFrame
  # https://stackoverflow.com/questions/43332057/pandas-strip-white-space
  df_respuesta[nombre_nueva_columna_excel_respuesta_1] = df_respuesta[nombre_nueva_columna_excel_respuesta_1].str.strip()
  df_respuesta[nombre_nueva_columna_excel_respuesta_2] = df_respuesta[nombre_nueva_columna_excel_respuesta_2].str.strip()
  df_respuesta[nombre_nueva_columna_excel_respuesta_3] = df_respuesta[nombre_nueva_columna_excel_respuesta_3].str.strip()
  df_respuesta[nombre_nueva_columna_excel_respuesta_4] = df_respuesta[nombre_nueva_columna_excel_respuesta_4].str.strip()


  # Reemplazar valores vacios y NULL en DataFrame de columnas en especifico

  # consulta_1 - max_altmora_a_corte_de_febrero_20_del_cliente_interna - casillas vacias o NULL se reemplazan por SIN CARTERA ACTIVA

  # reemplazar ESPACIOS EN BLANCO (CASILLAS VACIAS)
  # https://stackoverflow.com/questions/13445241/replacing-blank-values-white-space-with-nan-in-pandas
  df_respuesta[nombre_nueva_columna_excel_respuesta_1] = df_respuesta[nombre_nueva_columna_excel_respuesta_1].replace(r'^\s*$', 'SIN CARTERA ACTIVA', regex=True) 

  # reemplazar VALORES NULL
  diccionario_null_vacio_1 = {None    : "SIN CARTERA ACTIVA",
                              "None"  : "SIN CARTERA ACTIVA",
                              "NULL"  : "SIN CARTERA ACTIVA",
                              "null"  : "SIN CARTERA ACTIVA",
                              "NaN"   : "SIN CARTERA ACTIVA",
                              ""      : "SIN CARTERA ACTIVA"
                             }

  df_respuesta[nombre_nueva_columna_excel_respuesta_1] = df_respuesta[nombre_nueva_columna_excel_respuesta_1].replace(diccionario_null_vacio_1)

  df_respuesta[nombre_nueva_columna_excel_respuesta_1].fillna("SIN CARTERA ACTIVA", inplace=True)

  # Eliminar de los numeros del DataFrame el .0 q esta al final
  # Ejemplo: Si en el DataFrame hay un 1.0 entonces se reemplazara por 1 
  # y asi con todos los numeros q terminen en .0
  # https://stackoverflow.com/questions/46774069/python-remove-decimal-and-zero-from-string
  df_respuesta[nombre_nueva_columna_excel_respuesta_1] = df_respuesta[nombre_nueva_columna_excel_respuesta_1].astype(str).replace("\.0", "", regex=True)

  # consulta_2 - calificacion_al_momento_del_desembolso - casillas vacias o NULL se reemplazan por ERROR

  # reemplazar ESPACIOS EN BLANCO (CASILLAS VACIAS)
  df_respuesta[nombre_nueva_columna_excel_respuesta_2] = df_respuesta[nombre_nueva_columna_excel_respuesta_2].replace(r'^\s*$', "ERROR NO SE ENCONTRO INFORMACION", regex=True) 

  # reemplazar VALORES NULL
  diccionario_null_vacio_2 = {None    : "ERROR NO SE ENCONTRO INFORMACION",
                              "None"  : "ERROR NO SE ENCONTRO INFORMACION",
                              "NULL"  : "ERROR NO SE ENCONTRO INFORMACION",
                              "null"  : "ERROR NO SE ENCONTRO INFORMACION",
                              "NaN"   : "ERROR NO SE ENCONTRO INFORMACION",
                              ""      : "ERROR NO SE ENCONTRO INFORMACION"
                             }

  df_respuesta[nombre_nueva_columna_excel_respuesta_2] = df_respuesta[nombre_nueva_columna_excel_respuesta_2].replace(diccionario_null_vacio_2)

  df_respuesta[nombre_nueva_columna_excel_respuesta_2].fillna("ERROR NO SE ENCONTRO INFORMACION", inplace=True)

  # Eliminar de los numeros del DataFrame el .0 q esta al final
  df_respuesta[nombre_nueva_columna_excel_respuesta_2] = df_respuesta[nombre_nueva_columna_excel_respuesta_2].astype(str).replace("\.0", "", regex=True)


  # consulta_3 - max_calificacion_del_cliente_a_corte_de_junio_20 - casillas vacias o NULL se reemplazan por SIN CARTERA ACTIVA

  # reemplazar ESPACIOS EN BLANCO (CASILLAS VACIAS)
  df_respuesta[nombre_nueva_columna_excel_respuesta_3] = df_respuesta[nombre_nueva_columna_excel_respuesta_3].replace(r'^\s*$', "SIN CARTERA ACTIVA", regex=True) 

  # reemplazar VALORES NULL
  diccionario_null_vacio_3 = {None    : "SIN CARTERA ACTIVA",
                              "None"  : "SIN CARTERA ACTIVA",
                              "NULL"  : "SIN CARTERA ACTIVA",
                              "null"  : "SIN CARTERA ACTIVA",
                              "NaN"   : "SIN CARTERA ACTIVA",
  ...
  df_respuesta[nombre_nueva_columna_excel_respuesta_3] = df_respuesta[nombre_nueva_columna_excel_respuesta_3].replace(diccionario_null_vacio_3)

  df_respuesta[nombre_nueva_columna_excel_respuesta_3].fillna("SIN CARTERA ACTIVA", inplace=True)

  # Eliminar de los numeros del DataFrame el .0 q esta al final
  df_respuesta[nombre_nueva_columna_excel_respuesta_3] = df_respuesta[nombre_nueva_columna_excel_respuesta_3].astype(str).replace("\.0", "", regex=True)


  # consulta_4 - max_altamora_a_corte_de_febrero_20_del_cliente_externa - casillas vacias o NULL se reemplazan por SIN INFORMACION

  # reemplazar ESPACIOS EN BLANCO (CASILLAS VACIAS)
  df_respuesta[nombre_nueva_columna_excel_respuesta_4] = df_respuesta[nombre_nueva_columna_excel_respuesta_4].replace(r'^\s*$', "SIN INFORMACION", regex=True) 

  # reemplazar VALORES NULL
  diccionario_null_vacio_4 = {None    : "SIN INFORMACION",
  ...
  df_respuesta[nombre_nueva_columna_excel_respuesta_4] = df_respuesta[nombre_nueva_columna_excel_respuesta_4].replace(diccionario_null_vacio_4)

  df_respuesta[nombre_nueva_columna_excel_respuesta_4].fillna("SIN INFORMACION", inplace=True)

  # Eliminar de los numeros del DataFrame el .0 q esta al final
  df_respuesta[nombre_nueva_columna_excel_respuesta_4] = df_respuesta[nombre_nueva_columna_excel_respuesta_4].astype(str).replace("\.0", "", regex=True)
